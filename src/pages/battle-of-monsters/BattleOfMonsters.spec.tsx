...
describe('BattleOfMonsters', () => {
  ...
  it('should start fight after click on button', async () => {
    ...
    expect(screen.getByTestId('winner-display-container')).toBeInTheDocument();
    expect(screen.getByTestId('winner-text')).toBeInTheDocument();
  });

  it('should remove the winner text after clicking on a monster', async () => {
    await battleOfMonstersFactory();
    await act(() => screen.getByTestId('monster-1').click());
    await act(() => screen.getByTestId('start-battle-button').click());
    expect(screen.getByTestId('winner-text')).toBeInTheDocument();
    await act(() => screen.getByTestId('monster-1').click());
    expect(screen.queryAllByTestId('winner-text').length).toEqual(0);
  });
});
